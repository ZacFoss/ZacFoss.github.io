...
divContent.appendChild(orderedList);

// 3. Give your new ordered list the class "countries"
orderedList.classList.add("countries");

// 4. Design the following function to run on the click of a button from the index page
const btn = document.querySelector("button");
btn.addEventListener('click', getListOCountries);
function getListOCountries() {
    let finalCountryList = createNewList();
    finalCountryList.forEach(insertCountries);
}
...
function createNewList() {
    ...
    for(i=0; i < 25; i++) {
        let min = Math.ceil(0);
        let max = Math.floor(value.length);
        let randomNumber = Math.floor(Math.random() * (max - min) + min);
        newCountryList[i] = countries[randomNumber];
    }
...
function insertCountries() {
        ...
        listItem.innerText = this.name + ", " + this.code;
